...
# Numbers that can be called directly.
digitMap = {
    "day": "0",
    "Ichi": "1",
    "knee": "2",
...
grammarCfg.cmd.map = Item(
    {
        ...
        #"home": Key("home"),  # Conflicts with "yon" (4)
        "end": Key("end"),
        ...
        "(enter|return|turn) [<n>]": release + Key("enter:%(n)d"),
        "tab [<n>]": Key("tab:%(n)d"),
        ...
        "<letters> [<n>]": Key("%(letters)s/2:%(n)d"),
        "<char> [<n>]": Text("%(char)s"),
        "<digit> [<n>]": Text("%(digit)s"),
        ...
        "[<text>] (stupefy|go to sleep|cancel and sleep) [<text2>]": Function(cancel_and_sleep),  # @IgnorePep8
    },
    namespace={
        "Key": Key,
        "Text": Text,
